    ...
    UserCredentialsInterface
{
    ...
    {
        // $dsn = 'mysql:dbname=exdemos_dev_1500;host=localhost';
        // $username = 'exdemos_dev_1500';
        // $password = 'sdjk*(sdjsd';
        // Changed by Shaul. 
        // Class is hack on vendor , could be overridden if vendor is cleaned and  composer install is ran.  should be fixed.
        ...
        if (!$connection instanceof \PDO) {
            ...
            if (!is_array($connection)) {
                throw new \InvalidArgumentException('First argument to OAuth2\Storage\AetStorage must be an instance of PDO, a DSN string, or a configuration array');
            }
...
//    }
    ...
    {
        $fxuser = $this->tempGetUser($username);
        if(!empty($fxuser)){
//            var_dump($fxuser); exit;
            return $this->checkPassword($fxuser, $password);
        }
        return false;
    }
    ...
    {
        return $this->tempGetUser($username);
    }
...
//        return $user['password'] == sha1($password);
    ...
    public function tempGetUser($username)
    {
        $stmt = $this->db->prepare($sql = sprintf('SELECT * from %s where username LIKE :username', $this->config['user_table']));
